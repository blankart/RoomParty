...
export default memo(function VimeoVideoSearch(props: VideoSearchProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)}>
            <Input
              label={
                <>
                  Paste a <FaVimeo className="inline-block text-blue-200" />{" "}
                  <span className="text-blue-200">vimeo</span> video link
                </>
              }
              placeholder="https://vimeo.com/654598546"
              {...register("link", {
                required: "Vimeo video link is required",
                validate: (value) => {
                  if (!value?.match(/^https:\/\/(www\.)?vimeo\.com\//))
