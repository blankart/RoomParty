...
class Emitter {
    ...
    private generateKeyWithChannel<T>(module: string, channel: keyof T, key: string) {
            ...
            channel<Key extends keyof ChannelTypes>(channelKey: Key) {
                return {
                    on(key: string, cb: (args: ChannelTypes[Key]) => any) {
                        self.emitter.on(self.generateKeyWithChannel<ChannelTypes>(module, channelKey, key), cb)
                    },

                    off(key: string, cb: (args: ChannelTypes[Key]) => any) {
                        self.emitter.off(self.generateKeyWithChannel<ChannelTypes>(module, channelKey, key), cb)
                    },

                    emit(key: string, value: ChannelTypes[Key]) {
                        self.emitter.emit(self.generateKeyWithChannel<ChannelTypes>(module, channelKey, key), value)
                    }
                }
            }
        }
