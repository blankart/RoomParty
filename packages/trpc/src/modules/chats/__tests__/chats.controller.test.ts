...
describe("chats.controller", () => {
  ...
  test("chatSubscription", async () => {
    let maybeWillThrowError = async () =>
      await chatsController.chatSubscription(
        {
          id: "ID",
          localStorageSessionId: 1,
          name: "Name",
          roomTransientId: "ROOM_TRANSIENT_ID",
          password: "PASSWORD",
        },
        null
      );
    ...
    (roomServiceMock as any).isAuthorizedToEnterRoom.mockImplementation(
      () => true
    );
    ...
    ).client.roomTransient.findFirst.mockImplementation(() => ({}));
