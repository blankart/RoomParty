...
export const usersRouter = createRouter().query("googleOAuth", {
  async resolve({ ctx }) {
    const { token, redirect } = await UsersService.googleOAuth(
      ctx.req.query as { state: string; code: string; scope: string }
    );
